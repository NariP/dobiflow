...
- ⚠️ **An answer to a scope/approach question is NOT "approval."** Even if you asked about scope/approach via AskUserQuestion in steps 1·3 and
  got an answer, that's only a *design agreement*. **You must get a separate explicit OK to this step 4 "shall I implement it this way?"** before going
  to step 5 (implementation). Don't mistake a mid-question answer for approval and jump straight in.
...
- **Setup**: base = `{base_branch}` if injected (milestone mode = group branch), otherwise `{default_branch}`.
  A single task branches off that base as `{branch_prefix.feat}<slug>` (default `feat/`, an appropriate prefix if it's a refactor).
  Milestone mode only stacks commits on the group branch and creates no issue branch or PR (same as the triage-fix step 5·6 milestone guards).
...
Role boundary SSOT: `agents/git-writer.md` (main judges/writes; git-writer only executes, reading no code/log/diff).
...
`${CLAUDE_PLUGIN_ROOT}/docs/dobi-persona.md` (Read as needed).
