...
**서브에이전트 스폰 시 `{models}`가 있으면 `config.models[<agent>]`의 모델로 띄운다**(없으면 inherit — opt-in 오버라이드).
...
- **준비**: base = `{base_branch}` 주입 시 그것(마일스톤 모드=그룹 브랜치), 아니면 `{default_branch}`.
  단일 작업은 그 base에서 브랜치 `{branch_prefix.feat}<슬러그>`(기본 `feat/`, 리팩토링이면 적절한 prefix).
  마일스톤 모드는 그룹 브랜치에 커밋만 쌓고 이슈 브랜치·PR 안 만듦(triage-fix 5·6단계 마일스톤 가드 동일).
  config `worktree=true`면 **triage-fix 5단계 "worktree 준비" 분기 동일 적용** — `<repo>/.claude/worktrees/<이슈번호>`에
  ...
  Serena 활성화 경로도 worktree — 기본 false면 현행 그대로).
