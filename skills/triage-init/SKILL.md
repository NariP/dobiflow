...
// triage.config.json
{
  ...
  "models": {
    // Agent files keep model: inherit; this block overrides it (opt-in). If unspecified, inherit.
    // planning/judgment/verification roles = higher-tier model / judgment-free "hands" (git-writer, qa-runner) = lower. (Claude camp example — Codex uses gpt higher/lower)
    // qa (the audit) is an arbiter, so no downshift (keep the strong model). qa-runner only executes → haiku (scenario evaluation passed).
    // Dropping git-writer further to Haiku only after passing the per-op scenario evaluation.
    "planner": "opus", "implementer": "opus", "issue-triage": "opus",
    "code-reviewer": "opus", "policy-checker": "opus", "qa": "opus",
