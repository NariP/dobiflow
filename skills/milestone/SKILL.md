...
- **병렬 폭 `{milestone.max_parallel}`**(중지 모드면 1=순차). 각 그룹은 자기 worktree에서 태스크를 **순차**로:
  ...
  - **Serena 워커 정책(모드별 — 워커 스폰 프롬프트에 명시)**: **병렬(바이패스)** = 워커 **Serena 호출 금지
    ...
    혼동된다(워커는 planner 계획+search-cache로 탐색 수요가 낮아 실손 적음). **순차(중지)** = 워커가
    **자기 worktree 절대경로로 activate 후 사용 허용**(동시 사용자 없음) — 첫 질의 워밍업(수십 초/worktree) 비용 인지.
...
- **어느 모드든 main 머지는 사람이 한다.** 최종 PR 머지 후(사용자 "머지했어/정리해줘" 발화 — task-run 7단계와 동일 시점,
  ...
  3. **브랜치 정리** — 마일스톤 브랜치·잔여 그룹 브랜치 `op=cleanup-branch`(머지된 것만 — 7단계 sweep 안전선 동일).
