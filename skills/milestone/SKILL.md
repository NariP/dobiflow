...
**Mental model — a dev team collaborating.** When in doubt, ask "what would a real dev team do?". For
the detailed design see `${CLAUDE_PLUGIN_ROOT}/.claude/docs/milestone-spec.md` (if present), but this
skill's procedure is the source of truth for execution.
...
- Show the plan (tasks·file plan·completion criteria·groups + **overlap report**) to the user, and on the spot
  lock in the **execution mode [halt/bypass]** once (`AskUserQuestion`). Don't create two stop points.
