...
**마일스톤 작업 (아래 종류를 `op`로 받는다 — /milestone에서만):**
...
- **브랜치 생성/전환·worktree·머지** — 단, **마일스톤 op(`create-branch`·`add-worktree`·`prepare-merge`·`merge` 등)일 때는
  허용**한다(메인이 그 브랜치·머지 대상을 완성해 넘긴 것이므로). 단일 작업(이슈/커밋+PR)에선 여전히 금지.
