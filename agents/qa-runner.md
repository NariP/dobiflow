...
name: qa-runner
description: >-
  테스트·full_verify를 실행하고 결과를 verify.log로 남기는 실행 전담 에이전트. 판정하지
...
- The command to run (`test_command` for a task self-check, `full_verify_command` for pre-merge verification)
  and the **verify.log path** to write.
...
2. **Pre-merge verification (group PR ⑨ / final PR ⑩)**: run `full_verify` on the merge-candidate SHA (the
   combined commit M or the final milestone HEAD) → **merge/final verify.log**.
