...
| 5단계 자가체크 | **Fan-out (sectioning) + 2페이즈** | 정책 검사·코드 리뷰·테스트 실행(qa-runner)은 서로 독립이라 병렬(페이즈 A). 테스트 감사·판정(qa)만은 실행 결과가 있어야 의미가 있어 **페이즈 B로 뒤에** 둔다. 러너가 빨강이면 어차피 REQUEST_CHANGES라 그 라운드 감사는 스킵(fail-fast). 검사끼리는 **일부러 서로 안 보게** 해 판정 독립성을 지킴. |
...
qa-runner는 테스트 명령만 돌려 verify.log를 남기고, **소스·스냅샷·픽스처는 고치지 않는다**
