...
/work <무엇이든>
  ...
  │     │            Evaluator–optimizer
  │     │
  │     └─ ⊃ [fan-out]  자가체크 A: policy-checker + code-reviewer + qa-runner 병렬 (격리)
  │                     자가체크 B: 러너가 초록일 때만 qa 감사 (빨강이면 그 라운드 스킵)
  ...
  │
  └─ [순차·선택]      7. 머지 후 정리 — 사용자 발화로만 진입 (태깅·로컬 sweep·비용 리포트 ✋)
```
...
| 5단계 자가체크 | **Fan-out (sectioning) + 2페이즈** | 정책 검사·코드 리뷰·테스트 실행(qa-runner)은 서로 독립이라 병렬(페이즈 A). 이때 러너가 도는 건 **`test_command`(unit)뿐** — e2e는 느려서 매 라운드 돌릴 수 없으니 머지 게이트로 미룬다. 테스트 감사·판정(qa)만은 실행 결과가 있어야 의미가 있어 **페이즈 B로 뒤에** 둔다. 러너가 빨강이면 어차피 REQUEST_CHANGES라 그 라운드 감사는 스킵(fail-fast). 검사끼리는 **일부러 서로 안 보게** 해 판정 독립성을 지킴. |
| 5.5단계 부채 테스트 감사 | **게이트 (순차)** | APPROVE 후·커밋 전에 이번 루프가 추가한 테스트만 "깨지면 버그인가, 리팩토링인가"로 분류해 부채 테스트를 걸러냄. main에 부채가 안 들어가는 마지막 관문. |
| 7단계 머지 후 정리 (선택) | **Prompt chaining (순차)** | 사용자의 "머지했어" 발화로만 진입. 머지 사실 확인 → 태깅 → 로컬 sweep → 비용·시간 리포트(✋ 확인 후) 순서가 고정이고, 자동 진입하지 않아 미머지 브랜치를 지울 위험이 없음. 리포트는 말단 부가 단계라 실패해도 정리를 막지 않음(add-on). |
...
| 머지 전 검증 | **Merge-queue식 순차** | 그룹 브랜치를 하나씩 머지 → 매번 full verify(+ `e2e_command` 있으면 같이) → 최종 PR로 수렴. 조합 충돌을 머지 시점에 잡는다. e2e는 **루프 안이 아니라 여기서만** 돈다. |
...
### 리뷰 독립성이라는 숨은 강점 — 그리고 유일한 예외

`policy-checker`·`code-reviewer`·`qa`의 **판정끼리는 일부러 서로 안 보게** 한다.
버그가 아니라 설계다. 검사끼리 서로의 판정을 보면 "쟤가 통과시켰으니 나도"라는
담합·노이즈가 생긴다. 격리해야 각자 놓친 걸 잡는다. **격리가 곧 품질이다.**

예외는 딱 하나, **qa-runner → qa의 정보 흐름**이다. 이건 판정의 전달이 아니라
**사실(fact)의 전달**이다. 러너는 verdict를 내지 않고 "돌렸다/못 돌렸다 + 어떤 테스트가
실패했나"만 verify.log에 남기고, qa는 그 사실 위에서 **혼자** 판정한다. 담합의 재료인
"남의 결론"이 흐르지 않으니 독립성은 그대로다. 오히려 실행과 판정을 한 에이전트가 겸하면
자기가 만든 초록을 자기가 승인하는 구조가 되므로, **분리가 독립성을 더 강하게** 만든다
(qa에서 Bash를 뺀 것도 같은 이유 — 감사는 물리적으로 테스트를 못 돌린다).
