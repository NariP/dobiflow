...
### Fixed
- **세션 재개 시 누적 토큰 리셋(#77)** — Codex 세션을 나중에 이어서 하면
  `total_token_usage`가 0부터 다시 센다(`thread_id`는 유지되고 카운터만 리셋). 마지막 값만
  읽던 방식은 마지막 구간만 보고했다. 실측 185개 세션 중 리셋 6건, 과소 배율 1.30x~**3.73x** —
  70만 토큰 세션이 19만으로 보고되고 있었다. 누적값이 감소하는 지점을 구간 경계로 보고 각
  구간의 마지막 값을 필드별로 합산한다. 리셋 없는 세션은 구간이 1개라 값이 불변(179/185
  바이트 동일, 나머지 6개가 정확히 리셋 세션). 리셋 판정은 `total_tokens` 감소로 한다 —
  `thread_settings_applied` 마커는 실측상 감소 8건 중 2건만 동반해 4분의 3을 놓친다.
  `--since` 경로의 dedupe 키도 `(구간번호, total_tokens)`로 교체 — 기존 키가 카운터 단조성에
  의존해, 리셋 후 카운터가 이전 값을 다시 지나면 실제 턴이 버려졌다(합성 세션에서 600이 300으로
  계상되는 것을 재현). 검증: ccusage와 리셋 6건 전부 총량·필드별 일치, Claude 경로 바이트 동일.
...
### Fixed
- **중복 계상(#70)** — 한 API 응답이 여러 assistant 줄로 기록되며 `usage`가 반복된다(공식
  cost-tracking 문서에 기술된 동작). 그 줄들을 전부 더해 **실제의 약 1.9배**를 출력했다
  (리포트 $76 / 실제 $39.91). `message.id`(없으면 `requestId`) 기준으로 요청당 한 번만
  집계한다 — 메인 트랜스크립트와 비동기 `.output` 양쪽. 반복 줄 중에서는 **최댓값**을 취한다:
  실측상 `output_tokens`만 값이 달라지고(에이전트 `.output`에서 64건, 앞줄은 스트리밍 중간값)
  첫 줄을 취하면 output이 약 12% 과소 집계된다. 동기 `agentType` 경로는 제외 — tool-result
  줄엔 `message.id`가 없어 dedup 키가 launch 줄 `requestId`로 떨어지면 서로 다른 호출이
  뭉친다(실측상 이 경로엔 중복 없음). 검증은 `ccusage`(=`/usage`와 0.1% 일치) 대조로,
  총액 0.7%·토큰 4종 1% 이내 확인.

### Changed
- **집계 범위 명시(#70)** — 표에 `implementer`·`qa` 같은 에이전트 이름이 나열돼 dobiflow가
  그 비용을 다 쓴 것처럼 읽히던 문제. 세션 전체(다른 스킬·일반 대화 포함)임을 한 줄로 밝히고,
  `--since` 사용 시엔 그 시점 이후임을 표시한다. 사용자 언어에 맞춰 분기.
  스킬별 분해는 하지 않는다 — 공식 지원이 없고 `attributionSkill`은 스킬 문서를 읽은 줄에만
  붙어 실제 비용의 일부만 잡혀 오히려 오해를 키운다.
- **비유 단위를 빅맥으로 통일(#70)** — 기존 6단계(사탕·삼각김밥·커피·점심·치킨·회식)는 나라마다
  가격·체감이 달랐다. 빅맥은 어디서나 같은 물건이라 현지 가격을 몰라도 환산된다($6 기준,
  빅맥 지수). 반 개 미만은 별도 문구. 금액 계산에는 영향 없음.

## [1.3.1] - 2026-09-03

비용 리포트 정확도 수정 — 비동기 서브에이전트가 통째로 빠져 있던 문제.

### Fixed
- **비동기 서브에이전트 누락(#66)** — 백그라운드로 띄운 서브에이전트는 tool-result 줄에
  `agentType`·`usage`를 남기지 않고 `outputFile` 포인터만 남긴다. 기존 집계가 `agentType` 있는 줄만
  세서 비동기 에이전트를 전부 누락했다 — 실측 약 20% 과소 집계(리포트 $37.31 / 실제 약 $45).
  `outputFile` JSONL의 assistant 줄 `usage`를 합산하고 `attributionAgent`로 버킷한다(동기의
  `agentType`과 같은 형식이라 기존 정규화가 그대로 적용 — 같은 에이전트는 한 행으로 병합).
  재시도·재개된 에이전트는 id를 유지해 두 launch 줄이 같은 `outputFile`을 가리킬 수 있어,
  읽은 파일을 기억해 중복 집계를 막는다(qa 감사가 발견 — 합성 픽스처에서 $30이 $60으로 계상되던 것).
  동기 경로는 미변경(추가만, 삭제 0줄) — develop 대비 플래그 5종 출력 바이트 동일 확인.
  알려진 한계: `--since`는 launch 줄만 필터하고 `outputFile`은 전체를 합산한다.
