...
![version](https://img.shields.io/badge/version-1.4.1-lightgrey)
...
> ㅇㅋ
  ...
  🔁 구현 → 린트·테스트 → 자가체크 (도비가 스스로를 검사해요)
     └ code-reviewer + policy-checker + qa-runner(테스트) → qa 감사: 지적 없음 ✓
...
**Codex에서는 보여줄 수 있는 게 더 적고, 그 사실도 함께 적혀 나와요.** Codex는 공개된 단가가
없어서 금액 없이 토큰만 나오고, 세션 안에서 도는 에이전트는 그 세션 기록에 사용량이 합쳐져 있어서
나눌 수가 없어요 — 그래서 표가 에이전트별이 아니라 합계 한 줄이에요. 대신 Claude에는 없는 게 하나
붙어요: 현재 구간의 플랜 사용률.

로컬 세션 기록만 읽고, 아무 데도 전송하지 않아요. `~/.dobiflow/bin/dobiflow-cost`로
실행되며(install.sh가 설치) 없으면 조용히 생략돼서 정리를 막지 않아요.

> **v1.2.x에서 올리는 경우** — `~/.dobiflow/bin/`에 파일이 하나 추가돼서
> **`install.sh`를 다시 실행**해야 해요. 안 그러면 도비가 리포트를 물어보고도 조용히 넘어가요.
