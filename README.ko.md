...
> /work 대시보드 로고 눌렀는데 아무 데도 안 감
  ...
  ✋ 도비는 여기서 멈춰요  레포·계정 확인했어요. 고쳐도 될까요, 주인님?
...
| 여러 레포·계정 | 헷갈려서 엉뚱한 데 push | 이슈 보고 레포 판단, push 직전 계정 재확인 |
...
- **쓰기는 게이트를 통과해야 해요.** 이슈/PR 만들기 직전 active GitHub 계정을 재확인하고
  (엉뚱한 계정으로 올리는 것 방지), 코드를 건드리기 전 주인님 허락을 받아요.
