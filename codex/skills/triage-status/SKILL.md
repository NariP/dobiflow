...
- **Config summary** — one table of the model mapping (row per role: `planner` · `implementer` ·
  ...
  `{models}` shows as `inherit` — if unset, the session model is inherited), then short bullets for
  the main settings: `repo`, `default_branch`, `loop.max_iterations`,
  `loop.full_verify_command` (`none` if unset), `worktree`, `serena`, `milestone.base_branch`.
  End with the path of the config file that was read. **If there is no config file**, don't break —
  show just one line: "no config — `/triage-init` recommended".
