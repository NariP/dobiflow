...
(verbose `git log`/`diff`/`gh` output stays trapped in the subagent). Its role boundary is the SSOT in `codex/agents/git-writer.toml` (main judges/writes; git-writer only executes, reading no code/log/diff).
